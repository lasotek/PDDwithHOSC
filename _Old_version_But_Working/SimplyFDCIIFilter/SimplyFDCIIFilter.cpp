// SimplyFDCIIFilter.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		_CStrNodes Connections2({ "y1 = 0", "y2 = 0", "y3 = 3", "y4 = 4", "z1 = 5", "z2 = 6" });
		_CStrNodes Connections3({ "y1 = 0", "y2 = 0", "y3 = 5", "y4 = 6", "z1 = 8", "z2 = 7" });
		if (Manager.IncludeSubcircuit(hCircuit, "A", hSubModel, Connections1) < 0)
			PERFORM_ERROR;
		if (Manager.IncludeSubcircuit(hCircuit, "B", hSubModel, Connections2) < 0)
			PERFORM_ERROR;
		if (Manager.IncludeSubcircuit(hCircuit, "C", hSubModel, Connections3) < 0)
			PERFORM_ERROR;
