// SimplySKFloatingFilterUnPropGnd.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		if (hFloatVCVS < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "k", "p", "r", "k", "l", 0, 10, hFloatVCVS) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageFollower(hCircuit, "0", "r", "p", "0", hFloatVCVS) < 0)
			PERFORM_ERROR;
		if (Manager.ForceModelCalculation(hCircuit, hFloatVCVS) < 0)
			PERFORM_ERROR;
		// Floating VCVS with CFMB
		...
		if (Manager.SetCompPriority(hCircuit, 0) < 0)
			PERFORM_ERROR;
		// Becauase of some bug in the program (not easy to fix) it is batter to extract meta-vertices at the beggining
