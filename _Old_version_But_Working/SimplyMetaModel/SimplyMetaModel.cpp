...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		//	PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hModel, 0) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "A", "O", "0", "X", "Y", 0, 1000, hModel) < 0) //The groundned output op-amp as a nullor, needs the declaration of reference node.
			PERFORM_ERROR;
		//if (Manager.AddCapacitor)
		//if (Manager.AddResistor(hCircuit, "ro", "O'", "O", 10, hModel, false) < 0) // as for the main circuit, but with additional model handler
		//	PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hModel, 1) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R1", "I1", "X", 1e3, hModel, false) < 0) // as for the main circuit, but with additional model handler
			PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hModel, 3) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R2", "I2", "Y", 2e3, hModel, false) < 0) // as for the main circuit, but with additional model handler
			PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hModel, 4) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R3", "O", "Y", 3e3, hModel, false) < 0) // as for the main circuit, but with additional model handler
			PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hModel, 2) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R4", "I3", "X", 4e3, hModel, false) < 0)// as for the main circuit, but with additional model handler
			PERFORM_ERROR;
		if (Manager.ClearCompPriority(hCircuit, hModel) < 0)
			PERFORM_ERROR;
