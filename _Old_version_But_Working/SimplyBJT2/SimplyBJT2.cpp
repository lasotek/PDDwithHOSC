	...
	try
	{
		...
		if (!ONLY_C_DEPENDENT) {
			...
			if (S_EXPANDED)
			{
				...
				if (HFAlso)
				{
					if (Manager.AddCapacitor(hCircuit, "C_pi", "3", "5", 1e-6) < 0)
						PERFORM_ERROR;
					if (Manager.AddCapacitor(hCircuit, "C_mu", "3", "4", 1e-6) < 0)
						PERFORM_ERROR;
				}
			...
			else
			{
				if (NO_LF)
				{
					if (Manager.AddNullor(hCircuit, "1", "2", "1", "2") < 0)
						PERFORM_ERROR;
					if (Manager.AddNullor(hCircuit, "5", "7", "5", "7") < 0)
						PERFORM_ERROR;
					if (Manager.AddNullor(hCircuit, "6", "0", "6", "0") < 0)
						PERFORM_ERROR;
				}
				else
				{
					if (Manager.AddResistor(hCircuit, "sC_S_1", "1", "2", 1e-6, true) < 0)
						PERFORM_ERROR;
					if (Manager.AddResistor(hCircuit, "sC_S_2", "5", "7", 1e-6, true) < 0)
						PERFORM_ERROR;
				...
				if (HFAlso)
				{
					if (Manager.AddResistor(hCircuit, "sC_pi", "3", "5", 1e-6, true) < 0)
						PERFORM_ERROR;
					if (Manager.AddResistor(hCircuit, "sC_mu", "3", "4", 1e-6, true) < 0)
						PERFORM_ERROR;
				}
