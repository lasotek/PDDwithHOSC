...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		if (Manager.SetCompPriority(hCircuit, 1) <0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R_p", "3", "0", 1e3, false)<0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R_p", "4", "0", 1e3, false)<0)
			PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, 0) <0)
			PERFORM_ERROR;
		if (Manager.AddTransimpedance(hCircuit, "DR_p", "1", "5", "5", "3", 0, 1)<0)
			PERFORM_ERROR;
		if (Manager.AddTransimpedance(hCircuit, "DR_p", "6", "2", "6", "4", 0, 1)<0)
			PERFORM_ERROR;
