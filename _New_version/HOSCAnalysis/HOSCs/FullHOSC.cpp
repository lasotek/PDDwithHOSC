...
namespace HOSC
{
     ...
     * @return int 
     */
    int clear_canonical(int &source, int &target)
    {
        if (source == target)
            return 0;
        int sgn = 1;
    ...
    {
        sgn_ *= sgn;
        sgn_ *= clear_canonical(rsource, rtarget);
        sgn_ *= clear_canonical(csource, ctarget);
        row_deletions_.emplace_back(SingleDel(rsource, rtarget));
        col_deletions_.emplace_back(SingleDel(csource, ctarget));
    }

    HOSC::HOSC(int sgn, const vec_del_set &initial_dels, short max_nodes) : n_nodes_(max_nodes)
    {
    ...
    std::string HOSC::String(const std::vector<std::string> *pMaper) const
    {
        ...
        else
        {
            ...
            while (rit != row_deletions_.end() && cit != col_deletions_.end())
            {
                ...
                if (pMaper)
                {
                    out << "(" << (*pMaper)[p] << (r >= 0 ? "+" : "-") << (*pMaper)[std::abs(r)] << ")("
                        << (*pMaper)[k] << (l >= 0 ? "+" : "-") << (*pMaper)[std::abs(l)] << ")"
                        << ",";
                }
                else
                {
    ...
    HOSC::HOSC_oper_result HOSC::HOSC_big_dot(HOSC_oper_result h2, const nodes_to_remove &nodes) const
    {
        if (!h2)
            ...
            return {};
        auto n_nodes = std::min(n_nodes_, h2->n_nodes_);
        ...
        if (row_deletions_.size() + ext_row_deletions.size() > n_nodes)
            return {};
        if (row_deletions_.size() + ext_row_deletions.size() < nodes.size())
            return {};
        auto rem_nodes = Removing_pair_nodes(nodes);
        auto del_cleanear = [&rem_nodes](int &p, int &r, int &k, int &l) -> int /*sgn change*/
        {
            int lsgn = 1;
        ...
        HOSC_oper_result res = std::make_shared<HOSC>(n_nodes);
