// SimplySKFloatingFilterSub.cpp : Defines the entry point for the console application.
//
...
#define _CFMB
#define _ASYMETRIC false
#define _FLOAT_GROUND false
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		_CStrNodes BoundaryNodes({"1", "2", "7", "8"});
		long hWhole = Manager.CreateNewSubcircuit(hCircuit, "Whole", BoundaryNodes, nullptr, true);
		if (hWhole < 0)
			PERFORM_ERROR;

		//if (Manager.SetCompPriority(hCircuit, 0) < 0)
		//	PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "k", "7", "8", "5", "6", 0, 10, hWhole) < 0)
			PERFORM_ERROR;
		//if (Manager.ClearCompPriority(hCircuit) < 0)
		//	PERFORM_ERROR;
#ifdef _CFMB
		if (Manager.AddVoltageFollower(hCircuit, "0", "7", "8", "0", hWhole) < 0)
			PERFORM_ERROR;
#endif
		if (Manager.AddResistor(hCircuit, _ASYMETRIC ? "R1a" : "R1", "1", "3", 1e3, hWhole, false) < 0) //Resistor definition. false - not as a conductance. Initial value in case of some numerical computation
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, _ASYMETRIC ? "R1b" : "R1", "2", "4", 1e3, hWhole, false) < 0) //Resistor definition. false - not as a conductance. Initial value in case of some numerical computation
			PERFORM_ERROR;
		//if (Manager.SetCompPriority(hCircuit, 0) < 0)
		//	PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, _ASYMETRIC ? "R2a" : "R2", "3", "5", 1e3, hWhole, false) < 0) //Resistor definition. false - not as a conductance. Initial value in case of some numerical computation
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, _ASYMETRIC ? "R2b" : "R2", "4", "6", 1e3, hWhole, false) < 0) //Resistor definition. false - not as a conductance. Initial value in case of some numerical computation
			PERFORM_ERROR;
		//if (Manager.ClearCompPriority(hCircuit) < 0)
		//	PERFORM_ERROR;
		if (Manager.AddCapacitor(hCircuit, _ASYMETRIC ? "C1a" : "C1", "3", "7", 1e-9, hWhole) < 0) //Capacitor definition. Initial value in case of some numerical computation
			PERFORM_ERROR;
		if (Manager.AddCapacitor(hCircuit, _ASYMETRIC ? "C1b" : "C1", "4", "8", 1e-9, hWhole) < 0) //Capacitor definition. Initial value in case of some numerical computation
			PERFORM_ERROR;
		//if (Manager.SetCompPriority(hCircuit, 0) < 0)
		//	PERFORM_ERROR;
		if (Manager.AddCapacitor(hCircuit, _ASYMETRIC ? "C2a" : "C2", "5", _FLOAT_GROUND ? "g" : "0", 1e-9, hWhole) < 0) //VCVS definition. 0 is power of s operator to model some dynamic. Here - no dynamic
			PERFORM_ERROR;
		if (Manager.AddCapacitor(hCircuit, _ASYMETRIC ? "C2b" : "C2", "6", _FLOAT_GROUND ? "g" : "0", 1e-9, hWhole) < 0) //VCVS definition. 0 is power of s operator to model some dynamic. Here - no dynamic
			PERFORM_ERROR;
		//if (Manager.ClearCompPriority(hCircuit) < 0)
		//	PERFORM_ERROR;
		if (Manager.ForceModelCalculation(hCircuit, hWhole) < 0)
			PERFORM_ERROR;
		_CStrConnections Connections({"1 =1 ", "2 = 2", "7 = 7", "8 = 8"});
		if (Manager.IncludeSubcircuit(hCircuit, "M", hWhole, Connections) < 0)
			PERFORM_ERROR;
