// SimpModelControlledSourcesImprovedkRAsym.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		if (Manager.AddResistor(hCircuit, "R", "xp", "xn", 1e3, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "k1*R", "zp", "0", 1e3, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "k2*R", "0", "zn", 1e3, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
