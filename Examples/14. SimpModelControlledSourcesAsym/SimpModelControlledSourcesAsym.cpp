// SimpModelControlledSourcesAsym.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		//Test model
		_CStrNodes FBCFOANodesTest({ "0", "yp", "yn", "wp", "wn" });
		long hFBCFOATest = Manager.CreateNewSubcircuit(hCircuit, "FBCFOATest", FBCFOANodesTest, nullptr, true);
		if (hFBCFOATest < 0)
			PERFORM_ERROR;
		if (Manager.SetCompPriority(hCircuit, hFBCFOATest, 0) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "\\alpha_1", "xp'", "xn'", "yp", "yn", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "\\chi_1", "0", "xn'", "xp'", "0", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddCurrentGain(hCircuit, "\\beta_1", "zp", "0", "xp", "xp'", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddCurrentGain(hCircuit, "\\beta_2", "zn", "0", "xn", "xn'", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
#ifdef _CFMB_OUT
		if (Manager.AddVoltageGain(hCircuit, "\\alpha_2", "wp", "wn", "zp", "zn", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "\\chi_2", "0", "wn", "wp", "0", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
#else
		if (Manager.AddVoltageGain(hCircuit, "\\alpha_21", "wp", "0", "zp", "0", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddVoltageGain(hCircuit, "\\alpha_22", "wn", "0", "zn", "0", 0, 1, hFBCFOATest) < 0)
			PERFORM_ERROR;
#endif
		if (Manager.AddResistor(hCircuit, _SAME_RM ? "Rm" :"Rm1", "zp", "0", 10000, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, _SAME_RM ? "Rm" : "Rm2", "zn", "0", 10000, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.ClearCompPriority(hCircuit, hFBCFOATest) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "R", "xp", "xn", 1e3, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "k*R", "zp", "zn", 1e3, hFBCFOATest, false) < 0)
			PERFORM_ERROR;
		if (Manager.ForceModelCalculation(hCircuit, hFBCFOATest) < 0)
			PERFORM_ERROR;
		//Test model
		...
		if (Manager.AddVoltageFollower(hCircuit, "wn", "0", "zn", "0", hFBCFOA) < 0)
			PERFORM_ERROR;
#endif
		if (Manager.AddResistor(hCircuit, "Rm1", "zp", "0", 10000, hFBCFOA, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "Rm2", "zn", "0", 10000, hFBCFOA, false) < 0)
			PERFORM_ERROR;
		...
		if (Manager.AddVoltageGain(hCircuit, "\\alpha_22", "wn", "0", "zn", "0", 0, 1, hFBCFOA_TE) < 0)
			PERFORM_ERROR;
#endif
		if (Manager.AddResistor(hCircuit, "Rm1", "zp", "0", 10000, hFBCFOA_TE, false) < 0)
			PERFORM_ERROR;
		if (Manager.AddResistor(hCircuit, "Rm2", "zn", "0", 10000, hFBCFOA_TE, false) < 0)
			PERFORM_ERROR;
