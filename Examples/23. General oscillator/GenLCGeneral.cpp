...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		long long hCofactor_DB0 = Manager.CreateNewCofactor(hCircuit, { { { "B", "0" }, { "B", "0" } } });
		if (hCofactor_DB0 < 0)
			PERFORM_ERROR;

		long long hCofactor_DC0 = Manager.CreateNewCofactor(hCircuit, { { { "C", "0" }, { "C", "0" } } });
		if (hCofactor_DB0 < 0)
			PERFORM_ERROR;

		long long hCofactor_DBC = Manager.CreateNewCofactor(hCircuit, { { { "B", "C" }, { "B", "C" } } });
		if (hCofactor_DB0 < 0)
			PERFORM_ERROR;

		long long hCofactor_DBCx1 = Manager.CreateNewCofactor(hCircuit, { { { "B", "0" }, { "C", "0" } } });
		if (hCofactor_DBCx1 < 0)
			PERFORM_ERROR;

		long long hCofactor_DBCx2 = Manager.CreateNewCofactor(hCircuit, { { { "C", "0" }, { "B", "0" } } });
		if (hCofactor_DBCx2 < 0)
			PERFORM_ERROR;

		long long hCofactor_DB0C0 = Manager.CreateNewCofactor(hCircuit, { { { "B", "0" }, { "B", "0" } }, { { "C", "0" }, { "C", "0" } } });
		if (hCofactor_DB0 < 0)
			PERFORM_ERROR;
